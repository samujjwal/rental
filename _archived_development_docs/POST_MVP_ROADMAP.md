...
export class AnalyticsController {
  ...
  @Get('user-growth')
  @Roles('ADMIN')
  async getUserGrowth(@Query('period') period: 'day' | 'week' | 'month') {
    return this.analyticsService.getUserGrowth(period);
  }

  @Get('listing-performance')
  @Roles('ADMIN', 'OWNER')
  async getListingPerformance(@Query('listingId') listingId?: string) {
  ...
  async getCohortAnalysis(@Query('cohortType') cohortType: 'registration' | 'first_booking') {
