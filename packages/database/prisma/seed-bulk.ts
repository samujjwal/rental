...
async function generateListings(count: number, users: any[], categories: any[]) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    listings.push({
      title: randomChoice(listingTitles),
      description: randomChoice(listingDescriptions),
      ownerId: owner.id,
      categoryId: category.id,
      pricePerDay: randomFloat(25, 500),
      pricePerWeek: randomFloat(150, 3000),
      pricePerMonth: randomFloat(500, 8000),
      securityDeposit: randomFloat(100, 1000),
      ...
      insuranceRequirement: {
        minimumCoverage: randomFloat(1000, 5000),
        policyTypes: ['GENERAL_LIABILITY', 'PROPERTY_DAMAGE'],
      },
...
async function generateBookings(count: number, users: any[], listings: any[]) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    bookings.push({
      ...
      paymentStatus: randomChoice(['PENDING', 'PAID', 'PAID', 'REFUNDED']),
...
async function generateReviews(count: number, bookings: any[], users: any[]) {
  ...
  for (let i = 0; i < count; i++) {
    const booking = randomChoice(bookings);
    const reviewer = randomChoice(
      users.filter((u) => u.id === booking.renterId || u.id === booking.ownerId),
    );
    ...
    if (reviewer && reviewee) {
      reviews.push({
        ...
        type: reviewer.id === booking.renterId ? 'LISTING_REVIEW' : 'RENTER_REVIEW',
...
async function generateInsurancePolicies(count: number, users: any[], listings: any[]) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    policies.push({
      ...
      coverageAmount: randomFloat(1000, 10000),
      ...
      status: randomChoice([
        ...
        InsurancePolicyStatus.VERIFIED,
        InsurancePolicyStatus.EXPIRED,
      ]),
      verifiedAt: Math.random() > 0.5 ? randomDate(new Date(2024, 0, 1), new Date()) : null,
      verifiedBy:
        Math.random() > 0.5
          ? randomChoice(users.filter((u) => u.role === UserRole.ADMIN)).id
          : null,
      verificationNotes: Math.random() > 0.7 ? 'Policy verified successfully' : null,
    });
  }
