...
pnpm --filter @rental-portal/api test:cov       # + coverage report
...
pnpm --filter @rental-portal/api test:e2e       # Full E2E suite (needs running API)
pnpm --filter @rental-portal/api test:smoke     # Smoke-only E2E (needs running API)
pnpm --filter @rental-portal/api test:security  # Security / OWASP E2E (needs running API)

# ── Web ──────────────────────────────────────────────────
pnpm --filter @rental-portal/web test            # Vitest (unit + component)
pnpm --filter @rental-portal/web test:coverage   # + coverage
pnpm --filter @rental-portal/web e2e             # Playwright E2E

# ── Mobile ───────────────────────────────────────────────
pnpm --filter @rental-portal/mobile test         # jest-expo

# ── Root shortcuts ───────────────────────────────────────
pnpm test:smoke      # API smoke E2E
pnpm test:security   # API security E2E
pnpm test:property   # API property-based
pnpm test:chaos      # API chaos tests
pnpm test:cov        # All workspaces with coverage
```
