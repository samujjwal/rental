...
export class ChaosEngineeringService {
  ...
  private readonly delayConfig = {
    test: { pgSleep: 0.1, network: 0.1, query: 0.1, stabilization: 0.1, webhook: 0.1 },
    production: { pgSleep: 5, network: 5, query: 10, stabilization: 1, webhook: 60 },
  };
  ...
  constructor(
    ...
    this.isTestMode = process.env.NODE_ENV === 'test' || process.env.CHAOS_TEST_MODE === 'true';
  }

  private get delays() {
    return this.isTestMode ? this.delayConfig.test : this.delayConfig.production;
  }
  ...
  async injectDatabaseConnectionPoolExhaustion(): Promise<void> {
    ...
    const promises = [];
    const connectionCount = this.isTestMode ? 5 : 100;
    for (let i = 0; i < connectionCount; i++) {
      const promise = this.prisma.$queryRaw`SELECT pg_sleep(${this.delays.pgSleep})`;
      if (promise && typeof promise.catch === 'function') {
        promises.push(
          promise.catch((err: any) =>
            this.logger.debug(`Expected connection error: ${err.message}`),
          ),
        );
      } else {
        // In test mode, mock may not return a promise
  ...
  async injectSlowQueries(): Promise<void> {
    this.logger.warn(`[CHAOS] Injecting: Slow database queries (${this.delays.query}s delay)`);

    const timeout = this.isTestMode ? '10s' : '60s';
    await this.prisma.$executeRaw`SET statement_timeout = ${timeout}`;

    // Run expensive query
    const queryPromise = this.prisma.$queryRaw`
      SELECT pg_sleep(${this.delays.query})
    `;
  ...
  async injectDatabaseDeadlock(): Promise<void> {
    ...
    const delay = this.isTestMode ? 10 : 100;
  ...
  async injectHighLatency(): Promise<void> {
    this.logger.warn(`[CHAOS] Injecting: High network latency (${this.delays.network}s delay)`);

    const originalFetch = global.fetch;
    global.fetch = async (url: RequestInfo | URL, init?: RequestInit) => {
      await new Promise((r) => setTimeout(r, this.delays.network * 1000));
  ...
  async injectStripeWebhookDelay(): Promise<void> {
    const delayMs = this.isTestMode ? 100 : 60000;
    this.logger.warn(`[CHAOS] Injecting: Stripe webhook delay (${delayMs}ms)`);

    process.env.STRIPE_WEBHOOK_DELAY = delayMs.toString();
  }
    ...
    const scenarios = [
      {
        ...
        fn: () => this.runScenario(this.injectDatabaseConnectionPoolExhaustion.bind(this)),
      },
      {
        name: 'Slow Database Queries',
        fn: () => this.runScenario(this.injectSlowQueries.bind(this)),
      },
      {
        name: 'Redis Outage',
        fn: () =>
          this.runScenarioWithValidation(
      ...
      { name: 'Cache Stampede', fn: () => this.runScenario(this.injectCacheStampede.bind(this)) },
      {
        ...
        fn: () => this.runScenario(this.injectHighLatency.bind(this)),
      },
  ...
  private async runScenario(injectFn: () => Promise<void>): Promise<void> {
    await injectFn();
    await new Promise((r) => setTimeout(r, this.delays.stabilization * 1000));
  }
