...
export class AuditArchivalService {
  ...
  constructor(
    ...
    s3Client?: S3Client, // Optional injected S3 client for testing
  ) {
    const region = this.config.get('AWS_REGION', 'us-east-1');
    const accessKeyId = this.config.get('AWS_ACCESS_KEY_ID');
    const secretAccessKey = this.config.get('AWS_SECRET_ACCESS_KEY');

    // Use injected client if provided (for testing), otherwise create new
    this.s3 =
      s3Client ||
      new S3Client({
        region,
  ...
  async archiveOldLogs(
    ...
    while (hasMore && totalArchived < 100000) {
      // Safety limit
      try {
        ...
        // Small delay to prevent DB overload (skip in test mode)
    ...
    this.logger.log(
      `Audit archival complete: ${totalArchived} logs in ${batches} batches, ${errors} errors`,
    );
  ...
  async getArchiveMetadata(
    ...
    this.logger.log(
      'Archive metadata query not fully implemented - use Athena for querying archived logs',
    );
    return {
      totalBatches: 0,
      dateRange: {
