...
export class DatabasePerformanceService {
  ...
  getQueryStats(): {
    ...
    const slowQueries = this.queryHistory.filter(q => 
      q.duration > this.slowQueryThresholds.warning
    );
  ...
  getPerformanceRecommendations(): string[] {
    ...
    if (this.metrics.connectionPoolUsage > 0.8) { // 80% usage
      recommendations.push(
  ...
  private logSlowQuery(queryPerf: QueryPerformance): void {
    if (queryPerf.duration > this.slowQueryThresholds.critical) {
      this.logger.error(
        `🚨 CRITICAL SLOW QUERY: ${queryPerf.duration}ms`,
        {
      ...
      this.logger.warn(
        `⚠️ SLOW QUERY: ${queryPerf.duration}ms`,
        {
          query: queryPerf.query,
          duration: queryPerf.duration,
          parameters: queryPerf.parameters,
  ...
  private simulateConnectionPoolMetrics(): void {
    ...
    this.metrics.connectionPoolUsage = this.metrics.activeConnections / this.metrics.totalConnections;
  }
