...
describe('NotificationRetryService', () => {
  ...
  beforeEach(async () => {
    ...
    notificationService = module.get<InAppNotificationService>(InAppNotificationService);
    queueService = module.get<QueueService>(QueueService);
    configService = module.get<ConfigService>(ConfigService);
    logger = module.get<Logger>(Logger);
    
    // Clear all mocks to prevent pollution between tests
    jest.clearAllMocks();
  });
  ...
  describe('Queue Management', () => {
    ...
    it('should process retry queue', async () => {
      ...
      expect(notificationService.sendEmail).toHaveBeenCalledWith('test@example.com', 'Test message');
  ...
  describe('Different Notification Types', () => {
    it('should retry email notifications', async () => {
      ...
      expect(result.shouldRetry).toBe(true);
      expect(notificationService.sendEmail).toHaveBeenCalledWith('test@example.com', 'Test email');
    });
    ...
    it('should retry SMS notifications', async () => {
      ...
      notificationService.sendSMS.mockRejectedValueOnce(new Error('SMS gateway error'));

      const result = await retryService.retryNotification(notification);

      expect(result.shouldRetry).toBe(true);
      expect(notificationService.sendSMS).toHaveBeenCalledWith('+9771234567890', 'Test SMS');
    });
    ...
    it('should retry push notifications', async () => {
      ...
      notificationService.sendPush.mockRejectedValueOnce(new Error('Push service error'));

      const result = await retryService.retryNotification(notification);

      expect(result.shouldRetry).toBe(true);
      expect(notificationService.sendPush).toHaveBeenCalledWith('device-123', 'Test push');
    });
  ...
  describe('Notification Recovery Tests', () => {
    ...
    it('should handle manual retry triggers', async () => {
      ...
      // Act
      const result = await retryService.triggerManualRetry(notificationId, 'admin-456', 'Customer requested resend', notification);
    ...
    it('should validate manual retry permissions', async () => {
      ...
      // Act
      const result = await retryService.triggerManualRetry(notificationId, 'user-456', 'Unauthorized retry', notification);
    ...
    it('should handle notification reconciliation', async () => {
      ...
      // Act
      const result = await retryService.reconcileNotification(notificationId, notification);
    ...
    it('should handle notification recovery notifications', async () => {
      ...
      jest.spyOn(notificationService, 'sendEmail').mockResolvedValue({ messageId: 'email-123' });
      jest.spyOn(logger, 'info').mockImplementation(() => {});

      // Act
      await retryService.recoverNotification(notificationId, notification);
    ...
    it('should handle recovery failure notifications', async () => {
      ...
      jest.spyOn(notificationService, 'sendEmail').mockRejectedValue(new Error('Email service down'));
      jest.spyOn(logger, 'error').mockImplementation(() => {});

      // Act
      const result = await retryService.recoverNotification(notificationId, notification);
    ...
    it('should handle notification recovery with fallback channels', async () => {
      ...
      const notification = {
        ...
        body: 'Critical alert content',
        status: 'failed',
        fallbackChannels: ['sms', 'push'],
      };

      // Email fails, SMS succeeds
      jest.spyOn(notificationService, 'sendEmail').mockRejectedValue(new Error('Email server down'));
      jest.spyOn(notificationService, 'sendSMS').mockResolvedValue({ messageId: 'sms-123' });
    ...
    it('should handle notification recovery with multiple fallbacks', async () => {
      ...
      // All channels fail
      jest.spyOn(notificationService, 'sendEmail').mockRejectedValue(new Error('Email server down'));
      jest.spyOn(notificationService, 'sendSMS').mockRejectedValue(new Error('SMS gateway down'));
      jest.spyOn(notificationService, 'sendPush').mockRejectedValue(new Error('Push service down'));
