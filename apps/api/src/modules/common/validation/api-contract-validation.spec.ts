...
import { IsString, IsDate, IsNumber, IsOptional, IsDateString } from 'class-validator';
...
// Sample DTO classes for testing with proper decorators
...
describe('API Contract Validation Tests', () => {
  ...
  describe('SERIALIZATION VALIDATION', () => {
    ...
    it('should deserialize plain object to DTO', () => {
      ...
      // Note: class-transformer doesn't auto-convert date strings to Date objects
      // unless @Type(() => Date) decorator is used. For now, validate as strings.
      expect(typeof dto.startDate).toBe('string');
      expect(typeof dto.endDate).toBe('string');
      expect(new Date(dto.startDate)).toBeInstanceOf(Date);
      expect(new Date(dto.endDate)).toBeInstanceOf(Date);
    });
