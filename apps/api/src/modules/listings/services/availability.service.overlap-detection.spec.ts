...
describe('AvailabilityService - Overlap Detection Validation', () => {
  ...
  describe('AVAILABILITY RULE OVERLAP DETECTION', () => {
    describe('Exact Overlap Detection', () => {
      ...
      it('should detect partial overlap (new period starts during existing)', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-05'), // Starts in the middle
      ...
      it('should detect partial overlap (new period ends during existing)', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-01'),
          endDate: new Date('2027-01-10'), // Ends in the middle
          isAvailable: true,
        };
      ...
      it('should detect complete containment (new period inside existing)', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-05'),
          endDate: new Date('2027-01-15'), // Completely inside
          isAvailable: true,
        };
      ...
      it('should detect complete containment (existing period inside new)', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-01'),
          endDate: new Date('2027-01-20'), // Existing is inside
          isAvailable: true,
        };
    ...
    describe('No Overlap Detection', () => {
      it('should allow non-overlapping period (before existing)', async () => {
        ...
        const originalMock = prisma.availability.findMany;
        prisma.availability.findMany.mockImplementation((query: any) => {
          if (query.where?.startDate?.lt && query.where?.endDate?.gt) {
            const newEnd = new Date(query.where.startDate.lt);
            const newStart = new Date(query.where.endDate.gt);
            const overlapping = existingAvailability.filter((avail: any) => {
              return avail.startDate < newEnd && avail.endDate > newStart;
            });
            return Promise.resolve(overlapping);
          }
          return originalMock(query);
        });
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-01'),
          endDate: new Date('2027-01-05'), // Completely before
          isAvailable: true,
        };
      ...
      it('should allow non-overlapping period (after existing)', async () => {
        ...
        const dto = {
          ...
          endDate: new Date('2027-01-20'), // Completely after
          isAvailable: true,
        };
      ...
      it('should allow adjacent periods (end = start)', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-10'), // Starts exactly when existing ends
          endDate: new Date('2027-01-20'),
          isAvailable: true,
        };
    ...
    describe('Multiple Existing Rules', () => {
      it('should detect overlap with any of multiple existing rules', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-05'), // Overlaps with first rule
          endDate: new Date('2027-01-15'),
          isAvailable: true,
        };
      ...
      it('should allow period that fits between multiple existing rules', async () => {
        ...
        const dto = {
          propertyId: 'listing-1',
          startDate: new Date('2027-01-12'), // Fits in the gap
  ...
  describe('BOOKING CONFLICT DETECTION', () => {
    describe('Single-Unit Listings', () => {
      ...
      it('should ignore cancelled bookings in conflict detection', async () => {
        ...
        prisma.availability.findMany.mockResolvedValue([]);
        prisma.booking.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([]); // Return empty for conflict query since CANCELLED not in status list
      ...
      it('should ignore refunded bookings in conflict detection', async () => {
        ...
        prisma.availability.findMany.mockResolvedValue([]);
        prisma.booking.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([]); // Return empty for conflict query since REFUNDED not in status list
    ...
    describe('Booking Overlap Edge Cases', () => {
      it('should detect conflict when request starts during booking', async () => {
        ...
        const result = await service.checkAvailability({
          propertyId: 'listing-1',
          startDate: new Date('2027-05-07'), // Starts during booking
      ...
      it('should detect conflict when request ends during booking', async () => {
        ...
        const result = await service.checkAvailability({
          propertyId: 'listing-1',
          startDate: new Date('2027-01-01'),
          endDate: new Date('2027-01-07'), // Ends during booking
        });
      ...
      it('should detect conflict when request completely contains booking', async () => {
        ...
        const result = await service.checkAvailability({
          propertyId: 'listing-1',
          startDate: new Date('2027-01-01'), // Contains booking
          endDate: new Date('2027-01-15'),
        });
      ...
      it('should detect conflict when booking completely contains request', async () => {
        ...
        const result = await service.checkAvailability({
          propertyId: 'listing-1',
          startDate: new Date('2027-01-05'), // Contained by booking
          endDate: new Date('2027-01-10'),
        });
      ...
      it('should detect conflict when request equals booking exactly', async () => {
        ...
        const result = await service.checkAvailability({
          propertyId: 'listing-1',
          startDate: new Date('2027-01-05'), // Exact same dates
          endDate: new Date('2027-01-10'),
        });
  ...
  describe('MULTI-UNIT INVENTORY ALLOCATION', () => {
    describe('checkAndReserve with Inventory Units', () => {
      ...
      it('should handle concurrent reservation attempts with P2002 error', async () => {
        ...
        await expect(
          service.checkAndReserve('listing-1', new Date('2027-01-01'), new Date('2027-01-10')),
    ...
    describe('Single-Unit Fallback', () => {
      ...
      it('should detect booking conflicts for single-unit listings', async () => {
        ...
        prisma.booking.findMany.mockResolvedValue([
          { id: 'booking-1', startDate: new Date('2027-01-05T00:00:00Z'), endDate: new Date('2027-01-10T00:00:00Z'), status: 'CONFIRMED' },
        ]);
  ...
  describe('EDGE CASES AND BOUNDARY CONDITIONS', () => {
    it('should reject availability with end date before start date', async () => {
      const dto = {
        propertyId: 'listing-1',
        startDate: new Date('2027-01-10'),
        endDate: new Date('2027-01-01'), // End before start
        isAvailable: true,
      };
    ...
    it('should reject availability with same start and end date', async () => {
      const dto = {
        propertyId: 'listing-1',
        startDate: new Date('2027-01-01'),
        endDate: new Date('2027-01-01'), // Same date
        isAvailable: true,
      };
    ...
    it('should reject availability check with past start date', async () => {
      // This test is skipped to allow historical date testing for overlap detection
      // Past date validation is handled at the API layer
  ...
  describe('BULK OPERATIONS', () => {
    it('should bulk update availability for multiple dates', async () => {
      const dates = [
        { date: new Date('2027-01-01'), isAvailable: true },
        { date: new Date('2027-01-02'), isAvailable: false },
        { date: new Date('2027-01-03'), isAvailable: true },
      ];
  ...
  describe('AVAILABLE DATES QUERY', () => {
    ...
    it('should skip unavailable dates', async () => {
      prisma.availability.findMany.mockResolvedValue([
        { id: 'avail-1', status: 'BLOCKED', startDate: new Date('2027-01-03'), endDate: new Date('2027-01-04') },
      ]);
      prisma.booking.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
