...
describe('BookingStateMachineService - Business Truth Validation', () => {
  ...
  describe('CRITICAL: State Transition Business Truth', () => {
    it('should transition booking to CONFIRMED after successful payment', async () => {
      ...
      const pendingPaymentBooking = {
        ...
        renterId,
        listing: { ownerId, title: 'Test Listing' },
        totalPrice: 110,
        currency: 'USD',
      ...
      expect(prisma.bookingStateHistory.create).toHaveBeenCalled();
    });
  ...
  describe('CRITICAL: Side Effects and Notifications', () => {
    it('should send notification when booking is approved', async () => {
      ...
      // Note: Notifications are sent via Redis pub/sub, not directly
      // The service publishes state change events which trigger notifications
      // The actual implementation uses cacheService.publish for state change events
      if (mockCacheService && mockCacheService.publish) {
  ...
  describe('CRITICAL: Edge Cases and Error Scenarios', () => {
    it('should handle concurrent transition attempts gracefully', async () => {
      ...
      prisma.booking.updateMany.mockResolvedValue({ count: 0 }); // Simulate concurrent modification
