...
describe('Refund Calculation Scenario Tests', () => {
  ...
  describe('Renter-Initiated Cancellations', () => {
    it('should calculate full refund for early renter cancellation', async () => {
      ...
      // EXACT VALIDATION: Full refund including deposit (platform fee charged to owner, not refunded to renter)
  ...
  describe('Owner-Initiated Cancellations', () => {
    it('should calculate full refund for owner cancellation (owner penalty)', async () => {
      ...
      // EXACT VALIDATION: Full refund for owner cancellation (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBe(155); // 100 + 5 + 50 (basePrice + serviceFee + deposit)
      expect(result.penalty).toBe(0); // No penalty applied in owner cancellation
  ...
  describe('Dispute Resolution Refunds', () => {
    it('should calculate full refund for renter-favor dispute', async () => {
      ...
      // EXACT VALIDATION: Full refund for renter-favor dispute (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBe(155); // 100 + 5 + 50
      expect(result.penalty).toBe(0);
    });
  ...
  describe('Refunds with Discounts', () => {
    it('should calculate refund based on actual paid amount (not original price)', async () => {
      ...
      // EXACT VALIDATION: Refund based on actual paid amount (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBe(84); // 80 + 4 (basePrice + serviceFee)
      expect(result.penalty).toBe(0);
    });
    ...
    it('should handle partial refunds with pro-rated discounts', async () => {
      ...
      // EXACT VALIDATION: 50% of discounted amount (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBe(42); // 40 + 2 (50% of basePrice + serviceFee, excluding platform fee)
  ...
  describe('Multi-Day Booking Refunds', () => {
    it('should calculate refund for multi-day booking with partial usage', async () => {
      ...
      // EXACT VALIDATION: Pro-rated refund for unused days (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBeCloseTo(310.93, 1); // 248.5 + 12.43 + 50 (pro-rated basePrice + serviceFee + deposit)
      expect(result.depositRefund).toBe(50);
    });
  });
  ...
  describe('Edge Cases and Error Scenarios', () => {
    ...
    it('should handle very large booking amounts', async () => {
      ...
      // EXACT VALIDATION: Large amount handled correctly (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBe(12500); // 10000 + 500 + 2000 (basePrice + serviceFee + deposit)
      expect(result.penalty).toBe(0);
    });
  });
  ...
  describe('Currency Handling', () => {
    it('should handle different currencies correctly', async () => {
      ...
      // EXACT VALIDATION: Amount calculated in booking currency (platform fee charged to owner, not refunded)
      expect(result.refundAmount).toBe(155); // 100 + 5 + 50
      expect(result.penalty).toBe(0);
    });
  });
