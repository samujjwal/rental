...
describe('BookingCalculationService - Edge Cases & Comprehensive Tests', () => {
  ...
  describe('Edge Case: Fee calculations', () => {
    ...
    it('should calculate owner earnings correctly (subtotal - platform fee - service fee)', async () => {
      ...
      expect(result.ownerEarnings).toBe(result.subtotal * 0.85); // 100% - 15% (platform fee only, service fee charged to customer)
    });
  });
  ...
  describe('Edge Case: Total calculation', () => {
    it('should calculate total correctly (subtotal + platform fee + service fee + deposit)', async () => {
      ...
      const expectedTotal =
        result.subtotal + result.serviceFee + result.depositAmount; // platform fee charged to owner, not customer
  ...
  describe('Edge Case: Large numbers', () => {
    it('should handle very expensive listings', async () => {
      ...
      expect(result.ownerEarnings).toBe(59500); // 70000 - 10500 (service fee charged to customer, not deducted from owner earnings)
    });
  });
