...
// Mock user data - declared at top to avoid hoisting issues
const mockUser: any = {
  id: 'user-123',
...
describe('AuthService - 100% Coverage', () => {
  ...
  describe('Registration Flows', () => {
    test('should register user successfully with default role', async () => {
      ...
      // Mock event emitter
      eventEmitter.emit.mockReturnValue(undefined);
      ...
      expect(eventEmitter.emit).toHaveBeenCalledWith('user.registered', { userId: mockCreatedUser.id, email: mockCreatedUser.email });
    });
    ...
    test('should handle email queue failure gracefully', async () => {
      ...
      emailsQueue.add.mockRejectedValue(new Error('Queue failed'));
      eventEmitter.emit.mockReturnValue(undefined);
  ...
  describe('Login Flows', () => {
    ...
    test('should login successfully with MFA backup code', async () => {
      ...
      passwordService.verify.mockResolvedValue(true);
      fieldEncryption.decrypt.mockReturnValue('decrypted-secret');
      mfaService.verifyToken.mockResolvedValue(false); // TOTP fails
      tokenService.generateTokens.mockResolvedValue({
        accessToken: 'access-token',
        refreshToken: 'refresh-token',
      });
  ...
  describe('Password Reset Flows', () => {
    test('should initiate password reset successfully', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue(mockUser);
      tokenService.generatePasswordResetToken.mockResolvedValue(resetToken);
  ...
  describe('Email Verification', () => {
    ...
    test('should resend verification email successfully', async () => {
      ...
      await service.sendVerificationEmail(pendingUser.id);
  ...
  describe('MFA Management', () => {
    test('should enable MFA successfully', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue(mockUser);
      mfaService.generateSecret.mockReturnValue({ secret: mfaSecret, qrCode });
