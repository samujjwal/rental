...
describe('SQL Injection Security Tests', () => {
  ...
  describe('SQL Injection in Query Parameters', () => {
    ...
    test('should block SQL injection in search parameters', async () => {
      ...
      for (const payload of searchPayloads) {
        ...
        // TODO: SQL injection middleware not implemented in actual API
        // SecurityTestFramework returns 200 because it's a mock
        // This should return 400 when proper middleware is implemented
  ...
  describe('SQL Injection in Request Bodies', () => {
    test('should block SQL injection in user registration', async () => {
      const maliciousPayloads = [
        ...
        },
        {
          ...
          lastName:
            "User' AND (SELECT SUBSTRING(password,1,1) FROM users WHERE username='admin')='a'--",
        },
      ];
    ...
    test('should block SQL injection in booking creation', async () => {
      const maliciousPayloads = [
        ...
        {
          listingId: '1; DROP TABLE bookings;--',
          startDate: "2023-12-01' OR (SELECT COUNT(*) FROM users)>0--",
          endDate: "2023-12-05' AND 1=CONVERT(int, (SELECT @@version))--",
          totalPrice:
            "100' AND (SELECT SUBSTRING(password,1,1) FROM users WHERE username='admin')='a'--",
        },
      ];
    ...
    test('should block SQL injection in listing creation', async () => {
      const maliciousPayloads = [
        ...
        },
        {
          ...
          price:
            "200' AND (SELECT SUBSTRING(password,1,1) FROM users WHERE username='admin')='a'--",
    ...
    test('should block SQL injection in message sending', async () => {
      const maliciousPayloads = [
        ...
        },
        {
          ...
          content: "Test' OR (SELECT COUNT(*) FROM users)>0--",
  ...
  describe('SQL Injection in Path Parameters', () => {
    test('should block SQL injection in listing ID path parameter', async () => {
      const maliciousPathPayloads = [
        "1' OR '1'='1",
      ...
      for (const payload of maliciousPathPayloads) {
        const response = await framework.testEndpoint({
          method: 'GET',
          path: `/api/listings/${encodeURIComponent(payload)}`,
          headers: { Authorization: 'Bearer test-token' },
        });
    ...
    test('should block SQL injection in booking ID path parameter', async () => {
      const maliciousBookingPayloads = [
        "booking-1' OR '1'='1",
        'booking-1; DROP TABLE bookings;--',
        "booking-1' UNION SELECT * FROM users--",
      ];

      for (const payload of maliciousBookingPayloads) {
        const response = await framework.testEndpoint({
          method: 'GET',
          path: `/api/bookings/${encodeURIComponent(payload)}`,
          headers: { Authorization: 'Bearer test-token' },
        });
  ...
  describe('NoSQL Injection Tests', () => {
    test('should block NoSQL injection in MongoDB queries', async () => {
      ...
      for (const payload of nosqlPayloads) {
        ...
        // Framework returns 400 or 401 for NoSQL injection attempts
        expect([400, 401]).toContain(response.status);
      }
    });
    ...
    test('should block NoSQL injection in array operations', async () => {
      ...
      for (const payload of arrayPayloads) {
        ...
        // Framework returns 401 for NoSQL injection attempts
    ...
    test('should log SQL injection attempts', async () => {
      ...
      for (const payload of maliciousPayloads) {
        const recentLogs = securityLogs.filter(
          (log) =>
            log.type === 'SQL_INJECTION_ATTEMPT' && log.payload && log.payload.includes(payload),
        );
