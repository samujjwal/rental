...
describe('Rate Limiting Security Tests', () => {
  ...
  describe('API Rate Limiting', () => {
    test('should limit general API requests', async () => {
      ...
      // Make rapid requests with burst pattern to trigger rate limiting
      for (let i = 0; i < 150; i++) {
    ...
    test('should apply lenient limits to public endpoints', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        const response = await framework.testEndpoint({
          ...
          headers: { 'X-Request-ID': `public-req-${i}` }
        });
      ...
      expect(successfulRequests.length).toBeGreaterThanOrEqual(40);
      expect(rateLimitedRequests.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('User-Based Rate Limiting', () => {
    ...
    test('should handle concurrent user requests', async () => {
      ...
      expect(successfulRequests.length + rateLimitedRequests.length).toBeGreaterThan(0);
      expect(rateLimitedRequests.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('Burst Protection', () => {
    test('should detect and block suspicious IP patterns', async () => {
      ...
      // Should block suspicious IP more aggressively (framework returns 403 for suspicious IPs)
    ...
    test('should provide rate limiting metrics', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        await framework.testEndpoint({
          method: 'GET',
          path: '/api/test',
          headers: { 'X-Request-ID': `metrics-req-${i}` }
        });
      }
      ...
      expect(metrics.activeIps).toBeGreaterThanOrEqual(0);
      expect(metrics.activeUsers).toBeGreaterThanOrEqual(0);
      expect(metrics.averageRequestRate).toBeGreaterThanOrEqual(0);
  ...
  describe('Rate Limiting Security Validation', () => {
    test('should log rate limiting events', async () => {
      ...
      // Rate limiting events should be logged
