...
describe('🔥 Chaos Engineering Suite', () => {
  ...
  describe('Database Failure Scenarios', () => {
    ...
    it('should handle database schema migration conflicts', async () => {
      ...
      prisma.listing.findMany = jest.fn().mockRejectedValue(
        new Error('Schema mismatch: column does not exist')
      );
    ...
    it('should handle database index corruption', async () => {
      // Simulate index corruption
      prisma.listing.findMany = jest.fn().mockRejectedValue(
        new Error('Index corruption detected')
      );
    ...
    it('should handle database full table scans', async () => {
      // Simulate slow full table scan
      prisma.listing.findMany = jest.fn().mockImplementation(() => {
        return new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Query timeout: full table scan')), 300);
        });
      });
    ...
    it('should handle database foreign key constraint failures', async () => {
      // Simulate foreign key constraint violation
      prisma.booking.create = jest.fn().mockRejectedValue(
    ...
    it('should handle database check constraint failures', async () => {
      // Simulate check constraint violation (e.g., negative price)
      const response = await request(app.getHttpServer())
        .post('/listings')
        .send({
          title: 'Test Listing',
          basePrice: -100, // Negative price violates check constraint
          currency: 'NPR',
          categoryId: 'vehicles',
        });
    ...
    it('should handle database deadlock during concurrent updates', async () => {
      // Simulate deadlock
      prisma.listing.update = jest.fn().mockRejectedValue(
        new Error('Deadlock detected when trying to get lock')
      );

      const response = await request(app.getHttpServer())
        .patch('/listings/test-id')
        .send({ title: 'Updated' });
    ...
    it('should handle database write-ahead log (WAL) failures', async () => {
      // Simulate WAL failure
      prisma.$transaction = jest.fn().mockRejectedValue(
        new Error('WAL file corruption')
      );
    ...
    it('should handle database lock wait timeouts', async () => {
      // Simulate lock wait timeout
      prisma.listing.update = jest.fn().mockRejectedValue(
  ...
  describe('External Service Failure Scenarios', () => {
    it('should handle Stripe payment service timeouts', async () => {
      // Payment endpoint requires auth - register and login first
      ...
      if (registerResponse.status === 201) {
        ...
        if (loginResponse.status === 200) {
          const response = await request(app.getHttpServer())
            .post('/payments/intents/test-booking')
    ...
    it('should handle Stripe payment service connection failures', async () => {
      // Simulate Stripe connection failure by testing with invalid booking
      const registerResponse = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
          email: `stripe-conn-${Date.now()}@example.com`,
          password: 'Test123!@#',
          firstName: 'Test',
          lastName: 'User',
        });
      ...
      if (registerResponse.status === 201) {
        ...
        if (loginResponse.status === 200) {
          const response = await request(app.getHttpServer())
            .post('/payments/intents/invalid-booking')
            .set('Authorization', `Bearer ${loginResponse.body.token}`)
            .send({});
    ...
    it('should handle email service failures', async () => {
      // Email service failures should not prevent registration
      const response = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
      ...
      // Registration should succeed even if email fails (async email)
      expect([201, 400]).toContain(response.status);
    });

    it('should handle SMS service failures', async () => {
      // SMS service failures should not prevent operations
      const response = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
      ...
      // Registration should succeed even if SMS fails
      expect([201, 400]).toContain(response.status);
    });
    ...
    it('should handle storage service failures', async () => {
      ...
      // Should return 401 (auth required), not crash
    ...
    it('should handle third-party API service degradation', async () => {
      ...
      // Should still succeed even with degradation
      const successful = responses.filter(r => r.status === 'fulfilled' && r.value.status === 200).length;
      expect(successful).toBeGreaterThan(0);
    });

    it('should handle payment gateway timeout', async () => {
      // Payment operations should timeout gracefully
      const registerResponse = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
          email: `payment-timeout-${Date.now()}@example.com`,
          password: 'Test123!@#',
          firstName: 'Test',
          lastName: 'User',
        });
    ...
    it('should handle payment gateway invalid responses', async () => {
      // Simulate malformed payment gateway response
      const registerResponse = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
          email: `payment-invalid-${Date.now()}@example.com`,
          password: 'Test123!@#',
          firstName: 'Test',
          lastName: 'User',
        });
    ...
    it('should handle notification service failures', async () => {
      // Notification failures should not prevent operations
      const response = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
          email: `notif-fail-${Date.now()}@example.com`,
          password: 'Test123!@#',
          firstName: 'Test',
          lastName: 'User',
        });

      // Registration should succeed even if notifications fail
      expect([201, 400]).toContain(response.status);
    });

    it('should handle search service failures', async () => {
      // Search should degrade gracefully on failure
      const response = await request(app.getHttpServer())
        .get('/search?q=test');
    ...
    it('should handle geocoding service failures', async () => {
      // Geocoding failures should not prevent listing creation
      const registerResponse = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
          email: `geo-fail-${Date.now()}@example.com`,
          password: 'Test123!@#',
          firstName: 'Test',
          lastName: 'User',
        });
    ...
    it('should handle currency conversion service failures', async () => {
      // Currency conversion failures should not prevent operations
      const response = await request(app.getHttpServer())
        .get('/listings');

      // Should still return listings even if conversion fails
    ...
    it('should handle analytics service failures', async () => {
      ...
      // Should succeed even if analytics fails
      expect(response.status).toBe(200);
    });

    it('should handle webhook delivery failures', async () => {
      // Webhook failures should not cause transaction rollback
      const registerResponse = await request(app.getHttpServer())
        .post('/auth/register')
        .send({
      ...
      // Registration should succeed even if webhooks fail
      expect([201, 400]).toContain(registerResponse.status);
    });

    it('should handle CDN service failures', async () => {
      // CDN failures should fall back to local assets
      const response = await request(app.getHttpServer())
        .get('/listings');

      // Should succeed even if CDN fails
      expect(response.status).toBe(200);
    });
  });
