...
export function ModernTanStackForm<T extends Record<string, any>>({
    ...
    const renderField = (fieldConfig: FieldConfig) => {
        ...
        switch (fieldConfig.type) {
            case 'textarea':
                return (
                    <Box key={fieldConfig.name} sx={{ flex: `1 1 ${fieldConfig.gridColumn ? Math.min(fieldConfig.gridColumn * 4, 12) * 8.33 : 100}%`, minWidth: 0 }}>
                        <form.Field name={fieldConfig.name as any}>
                            {(fieldApi) => (
                                <TextField
                                    ...
                                    helperText={commonProps.helperText || fieldApi.state.meta.errors?.[0]}
                                />
                            )}
                        </form.Field>
                    </Box>
                        ...
                        <form.Field name={fieldConfig.name as any}>
                            {(fieldApi) => (
                                <TextField
                                    ...
                                    type="number"
                                    value={fieldApi.state.value ?? ''}
                                    onChange={(e) => fieldApi.handleChange(e.target.value === '' ? '' : Number(e.target.value) as any)}
                                    onBlur={() => fieldApi.handleBlur()}
                                    error={!!fieldApi.state.meta.errors?.length}
                                    helperText={commonProps.helperText || fieldApi.state.meta.errors?.[0]}
                                />
            ...
            case 'select':
                return (
                    <Box key={fieldConfig.name} sx={{ flex: `1 1 ${fieldConfig.gridColumn ? Math.min(fieldConfig.gridColumn * 4, 12) * 8.33 : 100}%`, minWidth: 0 }}>
                        <form.Field name={fieldConfig.name as any}>
                            {(fieldApi) => (
                                <FormControl fullWidth size="small" disabled={isFieldDisabled} required={fieldConfig.required}>
                                    <InputLabel>{fieldConfig.label}</InputLabel>
                                    <Select
                                        value={fieldApi.state.value ?? (fieldConfig.multiple ? [] : '')}
                                        onChange={(e) => fieldApi.handleChange(e.target.value as any)}
                                        onBlur={() => fieldApi.handleBlur()}
                                        multiple={fieldConfig.multiple}
                                        displayEmpty
                                        renderValue={fieldConfig.multiple ? (selected) => (
                                            ...
                                            if (!selected) return '';
                                            const option = fieldConfig.options?.find((opt) => opt.value === selected);
        ...
        <Box className={className}>
            ...
            <Box sx={{ mb: 3 }}>
                <Typography variant="h4" gutterBottom>
                    {title || `${mode === 'create' ? 'Create' : mode === 'edit' ? 'Edit' : 'View'} Item`}
                </Typography>
                    ...
                    {!isReadOnly && (
                        <Box sx={{ mt: 3, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                            ...
                            <Button
                                ...
                                disabled={loading}
                            >
                                {loading ? 'Saving...' : (submitText || (mode === 'create' ? 'Create' : 'Save'))}
                            </Button>
                        </Box>
                    )}
                </form>
