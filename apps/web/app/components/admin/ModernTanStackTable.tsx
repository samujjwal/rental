...
const EditableCell: React.FC<EditableCellProps> = ({ value, row, column, onUpdate, type }) => {
    ...
    return (
        ...
        >
            {column.columnDef.cell?.({ getValue: () => value, row, column }) || String(value || '')}
        </Box>
    );
};
...
export function ModernTanStackTable<T extends Record<string, any>>({
    ...
    // UI State
    const [columnVisibilityAnchorEl, setColumnVisibilityAnchorEl] = useState<null | HTMLElement>(null);
    ...
    const enhancedColumns = useMemo(() => {
            ...
            return cols.map((col) => ({
                ...
                cell: ({ getValue, row, column }: any) => {
                    ...
                    if (originalCell && typeof originalCell === 'function') {
                        return originalCell({ getValue, row, column, cell: column as any, renderValue: getValue, table: table as any });
                    }

                    return (
                        <Box
                            sx={{ cursor: enableInlineEditing && onRowEdit ? 'pointer' : 'default' }}
                            onClick={() => {
        ...
        return cols;
    }, [columns, enableRowSelection, rowActions, enableInlineEditing, onRowEdit, editingCell]);
    ...
    const handleBulkAction = async (action: BulkAction) => {
        ...
        const selectedRecords = selectedIds.map(id => data.find(row => row.id === id)).filter(Boolean);
        ...
        <Box className={className}>
                    ...
                    {filterFields.length > 0 && (
                        ...
                        >
                            ...
                            {columnFilters.length > 0 && (
                                <Badge badgeContent={columnFilters.length} color="primary" sx={{ ml: 1 }}>
                                    <Box />
                                </Badge>
                            )}
                        </Button>
                ...
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                    ...
                    {bulkActions.map((action) => (
                        <Button
                            ...
                            disabled={action.requiresSelection && Object.keys(rowSelection).length === 0}
                            ...
                            {filterFields.map((field) => (
                                <Box key={field.id} sx={{ minWidth: 200 }}>
                                    <FormControl fullWidth size="small">
                                        ...
                                        {field.type === 'select' ? (
                                            <Select
                                                value={columnFilters.find(f => f.id === field.id)?.value || ''}
                                                onChange={(e) => {
                                                    const existingFilter = columnFilters.find(f => f.id === field.id);
                                                    if (existingFilter) {
                                                        ...
                                                        handleColumnFiltersChange([...columnFilters, { id: field.id, value: e.target.value }]);
                                                    }
                                                }}
                                            >
                                            ...
                                            {header.column.getCanSort() && (
                                                <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                                                    <ArrowUpwardIcon
                                                        sx={{
                                                            fontSize: 12,
                                                            color: header.column.getIsSorted() === 'asc' ? 'primary.main' : 'grey.400',
                                                            opacity: header.column.getIsSorted() === 'desc' ? 0.3 : 1,
                                                        }}
                                                    />
                                                    <ArrowDownwardIcon
                                                        sx={{
                                                            fontSize: 12,
                                                            color: header.column.getIsSorted() === 'desc' ? 'primary.main' : 'grey.400',
                                                            opacity: header.column.getIsSorted() === 'asc' ? 0.3 : 1,
                                                        }}
                                                    />
                                                </Box>
                                            )}
            ...
            {enablePagination && (
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2 }}>
                    <Typography variant="body2" color="text.secondary">
                        Showing {table.getRowModel().rows.length} of {manualPagination ? totalCount : table.getFilteredRowModel().rows.length} results
                        {manualPagination && totalCount && ` (${totalCount} total)`}
                    </Typography>
                    <TablePagination
                        component="div"
                        count={manualPagination ? totalCount ?? data.length : table.getFilteredRowModel().rows.length}
                        page={manualPagination ? pagination.pageIndex : table.getState().pagination.pageIndex}
                        onPageChange={(_, page) => {
                            if (manualPagination) {
                ...
                <MenuList dense>
                    <MenuItemComponent onClick={() => {
                        Object.keys(columnVisibility).forEach((key) => {
                            handleColumnVisibilityChange({ ...columnVisibility, [key]: true });
                        });
                    ...
                    <MenuItemComponent onClick={() => {
                        Object.keys(columnVisibility).forEach((key) => {
                            handleColumnVisibilityChange({ ...columnVisibility, [key]: false });
                        });
                        setColumnVisibilityAnchorEl(null);
                    }}>
                        <ListItemIcon>
            ...
            >
                <MenuList dense>
                    ...
                    {onRowDelete && (
                        <MenuItemComponent onClick={() => {
                            if (selectedRow && window.confirm('Are you sure you want to delete this record?')) {
