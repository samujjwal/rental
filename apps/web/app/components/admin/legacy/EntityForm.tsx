...
export function EntityForm<T extends Record<string, any> = any>({
    ...
    const validateFormInternal = useCallback((data: Record<string, any>): boolean => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (mode === 'create' && entityConfig.hooks?.beforeCreate) {
                const hookedData = await entityConfig.hooks.beforeCreate(transformedData);
                Object.assign(transformedData, hookedData);
            } else if (mode === 'edit' && entityConfig.hooks?.beforeUpdate) {
                const hookedData = await entityConfig.hooks.beforeUpdate(data?.id || '', transformedData);
                Object.assign(transformedData, hookedData);
            }
            ...
            if (mode === 'create') {
                ...
                entityConfig.fields.forEach(field => {
                    resetData[field.key as string] = field.defaultValue ?? getDefaultValueForType(field.type);
                });
            ...
            if (entityConfig.hooks?.onError) {
                entityConfig.hooks.onError(err as Error, mode === 'create' ? 'create' : 'edit');
            }
        }
    };
    ...
    const renderField = (field: FieldConfig) => {
        ...
        // Check visibility
        if (typeof field.hidden === 'function' ? field.hidden(mode, data as T) : field.hidden) {
            return null;
        }
        ...
        const isFieldReadOnly = typeof field.readOnly === 'function'
            ...
            case 'number':
                return (
                    <TextField
                        ...
                        type="number"
                        value={value ?? ''}
                        onChange={(e) => handleFieldChange(field.key as string, e.target.value === '' ? '' : Number(e.target.value))}
                        placeholder={field.placeholder}
                        required={field.validation?.required}
                        inputProps={{
                        ...
                        <FormControlLabel
                            control={
                                <Switch
                                    ...
                                    onChange={(e) => handleFieldChange(field.key as string, e.target.checked)}
                                    disabled={isFieldDisabled}
                                    readOnly={isFieldReadOnly}
                                />
                            }
    ...
    const renderSections = () => {
        ...
        return entityConfig.formSections.map((section) => (
            <Accordion
                ...
                onChange={(_, expanded) => {
                    setExpandedSections(prev => ({ ...prev, [section.title]: expanded }));
                }}
        ...
        <Box>
            ...
            <Box sx={{ mb: 3 }}>
                <Typography variant="h4" gutterBottom>
                    {mode === 'create' ? `Create ${entityConfig.name}` :
                        mode === 'edit' ? `Edit ${entityConfig.name}` :
            ...
            <Box sx={{ mb: 3 }}>
                <Chip
                    icon={mode === 'edit' ? <EditIcon /> : mode === 'view' ? <ViewIcon /> : <AddIcon />}
                    label={mode === 'create' ? 'Creating New' : mode === 'edit' ? 'Editing' : 'View Only'}
            ...
            >
                <Alert severity="success" onClose={() => setShowSuccess(false)}>
                    {mode === 'create' ? `${entityConfig.name} created successfully` :
...
function transformDataToApi(data: Record<string, any>, fields: FieldConfig[]): Record<string, any> {
    ...
    fields.forEach(field => {
        ...
        switch (field.type) {
            case 'date':
            case 'datetime':
                result[field.key as string] = value ? new Date(value).toISOString() : null;
                break;
            case 'number':
            ...
            case 'json':
                try {
                    result[field.key as string] = typeof value === 'string' ? JSON.parse(value) : value;
                } catch {
