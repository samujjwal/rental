...
import { Box, Typography, Table, TableHead, TableRow, TableCell, TableBody, TableContainer } from '@mui/material';
import {
    useReactTable,
    getCoreRowModel,
    getSortedRowModel,
...
export function TanStackTable({ data, columns, totalCount }: TanStackTableProps) {
    console.log('TanStackTable props:', { data: data.slice(0, 2), columns: columns.slice(0, 2), totalCount });
    ...
    return (
        <Box>
            <Typography variant="body2" sx={{ mb: 2 }}>
                TanStackTable: Rendering {data.length} rows with {columns.length} columns
                {sorting.length > 0 && ` | Sorted by: ${sorting[0].id} ${sorting[0].desc ? 'desc' : 'asc'}`}
            </Typography>
            
            <Typography variant="body2" sx={{ mb: 1, color: 'gray' }}>
                Debug: Header groups: {table.getHeaderGroups().length}, 
