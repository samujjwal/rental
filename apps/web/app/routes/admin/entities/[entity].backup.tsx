...
export default function DynamicEntityPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch ${entityConfig.pluralName}`);
      }

      const result = await response.json();

      // Transform data if transformer is defined
      const transformedData = entityConfig.transformers?.list
        ? entityConfig.transformers.list(result.data || result)
        : result.data || result;
        ...
        setTableState((prev) => ({
          ...
          pagination: {
            ...
            totalPages:
              ...
              Math.ceil(result.pagination.total / prev.pagination.limit),
          },
        }));
      }
    } catch (err) {
    ...
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="error">
          Entity configuration not available. Please check the URL or contact an
          administrator.
        </Alert>
      </Box>
    );
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">{entityConfig.pluralName}</Typography>
      </Breadcrumbs>
