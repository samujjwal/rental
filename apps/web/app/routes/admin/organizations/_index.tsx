...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    try {
        ...
        const orgsData = response.ok ? await response.json() : { data: [], total: 0 };
        ...
        return {
            ...
            stats: {
                total: orgsData.total || 0,
                verified: (orgsData.data || []).filter((o: any) => o.isVerified).length,
                totalListings: (orgsData.data || []).reduce((sum: number, o: any) => sum + (o.stats?.listingsCount || 0), 0),
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                    row.isVerified ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                }`}>
