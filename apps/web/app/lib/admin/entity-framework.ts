...
export interface FieldConfig<T = any> {
  ...
  // UI configuration
  hidden?: boolean | ((mode: ViewMode, data: T) => boolean);
  disabled?: boolean | ((mode: ViewMode, data: T) => boolean);
  readOnly?: boolean | ((mode: ViewMode, data: T) => boolean);
  ...
  // Custom rendering
  render?: (value: any, record: T, mode: ViewMode) => ReactNode;
  renderForm?: (props: {
    value: any;
...
export interface RelatedSection<T = any> {
  id: string;
  title: string;
  entity: string;
  foreignKey: string;
  ...
  actions?: RowAction[];
  emptyMessage?: string;
  fetchData: (parentId: string) => Promise<any[]>;
}
...
class EntityRegistry {
  ...
  register<T>(config: EntityConfig<T>): void {
    this.entities.set(config.slug, config);
  }

  get<T>(slug: string): EntityConfig<T> | undefined {
...
export function registerEntity<T>(config: EntityConfig<T>): void {
  entityRegistry.register(config);
}

export function getEntityConfig<T>(slug: string): EntityConfig<T> | undefined {
...
export function createIdColumn<T>(): MRT_ColumnDef<T> {
  return {
...
export function createStatusColumn<T>(
  ...
  return {
    ...
    Cell: ({ row }) => {
      ...
      return { type: "chip", label: value, color };
    },
  } as MRT_ColumnDef<T>;
}
...
export interface GenericDataFormProps<T = any> {
  entityConfig: EntityConfig<T>;
...
export interface GenericDetailViewProps<T = any> {
  entityConfig: EntityConfig<T>;
  data: T;
  loading?: boolean;
  onEdit: () => void;
